    ...
    <div className="space-y-8 animate-fade-in font-sans text-zinc-900 dark:text-zinc-100">
      ...
      <div className="flex flex-col xl:flex-row gap-4 items-center">
        ...
        <div className="relative flex-1 w-full">
          <i className="fa-solid fa-magnifying-glass absolute left-3.5 top-1/2 -translate-y-1/2 w-4.5 h-4.5 text-zinc-400" ></i>
          <input 
            type="text" 
            value={searchTerm}
      ...
      {mobileDetailOpen && selectedJob && (
        <div className="lg:hidden fixed inset-x-0 bottom-0 top-20 z-[999] bg-white dark:bg-zinc-950 flex flex-col animate-in slide-in-from-bottom duration-300 shadow-2xl">
          {/* Drawer Header */}
          <div className="flex items-center justify-between px-3 py-2.5 border-b border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-950 shrink-0 shadow-sm">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0 pl-1">
                <p className="text-[9px] font-black uppercase tracking-widest text-emerald-500 leading-none">Job Details</p>
                <h2 className="text-xs font-extrabold text-zinc-900 dark:text-white truncate">{selectedJob.company}</h2>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              <button 
                onClick={(e) => {
                  e.stopPropagation();
                  if (appliedJobs[selectedJob.id]) {
                    handleResetApplyStatus(selectedJob.id);
                  } else {
                    const updated = { ...appliedJobs, [selectedJob.id]: true };
                    setAppliedJobs(updated);
                    if (user) localStorage.setItem(`applied_internships_${user.uid}`, JSON.stringify(updated));
                  }
                }}
                ...
                title={appliedJobs[selectedJob.id] ? "Marked as Applied" : "Mark as Applied"}
              >
          ...
          <div className="flex-1 overflow-y-auto pb-28">
            <div className="p-4 space-y-6">
              {/* Job Header */}
              <div className="space-y-3 pb-4 border-b border-zinc-200/50 dark:border-zinc-800/40">
                <div className="flex items-center justify-between">
                  <div className={`w-12 h-12 rounded-xl flex-shrink-0 bg-gradient-to-tr ${selectedJob.logo} text-white flex items-center justify-center font-black text-xl shadow-sm`}>
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  <span className="text-[10px] font-black uppercase tracking-wider px-2 py-1 bg-zinc-150 dark:bg-zinc-900 text-zinc-600 dark:text-zinc-400 border border-zinc-200/60 dark:border-zinc-800 rounded-md">
                    Stipend: {selectedJob.stipend}
                  </span>
                  {selectedJob.isHot && (
                    <span className="text-[10px] font-black uppercase tracking-wider px-2 py-1 bg-rose-500/10 text-rose-500 border border-rose-550/15 rounded-md">
                      Hot Role
                    </span>
                  )}
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <ul className="space-y-2">
                    {selectedJob.responsibilities.map((resp, idx) => (
                      <li key={idx} className="flex items-start gap-2.5 text-xs text-zinc-600 dark:text-zinc-400 font-medium leading-relaxed">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {selectedJob.tags.map((tag, idx) => (
                    <span 
                      key={idx} 
                      className="px-2.5 py-1 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200/60 dark:border-zinc-800 text-[11px] font-bold rounded-xl text-zinc-600 dark:text-zinc-400"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              <div className="border-t border-zinc-200/50 dark:border-zinc-800/40 pt-5 pb-2">
                {appliedJobs[selectedJob.id] ? (
                  <div className="flex items-center gap-2 w-full">
                    <span 
                      className="flex items-center justify-center flex-1 gap-1.5 px-4 py-3 bg-emerald-500/10 text-emerald-500 rounded-xl text-xs font-bold border border-emerald-500/20"
                    >
                      <i className="fa-solid fa-circle-check w-4.5 h-4.5" ></i>
                      Applied
                    </span>
                    <button 
                      onClick={() => handleResetApplyStatus(selectedJob.id)}
                      className="px-4 py-3 bg-zinc-100 hover:bg-zinc-200 dark:bg-zinc-900 dark:hover:bg-zinc-800 text-zinc-400 hover:text-rose-500 rounded-xl text-xs font-bold transition-all border border-zinc-200/50 dark:border-zinc-800/40 cursor-pointer"
                    >
                      Reset
                    </button>
                  </div>
                  ...
                  <button 
                    ...
                    className="w-full py-3.5 bg-zinc-950 dark:bg-white text-white dark:text-zinc-900 rounded-xl text-sm font-black hover:shadow-md active:scale-95 transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 border border-transparent dark:border-white/10"
                  >
              ...
              return (
                ...
                >
                      ...
                      <button 
                        onClick={(e) => {
                          ...
                          if (appliedJobs[job.id]) {
                            handleResetApplyStatus(job.id);
                          } else {
                            const updated = { ...appliedJobs, [job.id]: true };
                            setAppliedJobs(updated);
                            if (user) localStorage.setItem(`applied_internships_${user.uid}`, JSON.stringify(updated));
                          }
                        }}
                        className={`p-1 rounded-lg border transition-all cursor-pointer ${
                          appliedJobs[job.id] 
                            ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/30" 
                            : "bg-zinc-100 dark:bg-zinc-900 text-zinc-400 border-zinc-200 dark:border-zinc-800 hover:text-emerald-500 hover:bg-emerald-500/10 hover:border-emerald-500/30"
                        }`}
                        title={appliedJobs[job.id] ? "Marked as Applied" : "Mark as Applied"}
                      >
                        <i className="fa-solid fa-circle-check w-3.5 h-3.5"></i>
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="lg:hidden flex items-center justify-end gap-1 mt-2.5 text-[9px] font-black uppercase tracking-wider text-indigo-500/70">
                    ...
                    <i className="fa-solid fa-arrow-up-right-from-square w-3 h-3" ></i>
                  </div>
                </div>
              );
            })
        ...
        <div className="lg:col-span-7">
          {selectedJob ? (
              ...
              <div className="border-t border-zinc-200/50 dark:border-zinc-800/40 pt-5 flex items-center justify-between">
                ...
                {appliedJobs[selectedJob.id] ? (
                  ...
                  >
                    {applyingJobId === selectedJob.id ? (
                      <>
                        <i className="fa-solid fa-spinner fa-spin w-3.5 h-3.5 animate-spin" ></i>
                        Redirecting to Apply...
                      </>
                    ) : (
                      <>
                        Apply to Position
                        <i className="fa-solid fa-arrow-up-right-from-square w-3.5 h-3.5" ></i>
                      </>
                    )}
                  </button>
                )}
            ...
            <div className="glass-panel p-20 rounded-3xl text-center border-dashed border-zinc-300 dark:border-zinc-850 flex flex-col items-center justify-center">
              <i className="fa-solid fa-briefcase w-12 h-12 text-zinc-400 animate-bounce mb-4" ></i>
